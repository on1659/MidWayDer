...
### 4. 다국어 지원 🌐 (v0.48.0+)
- **한국어/영어 UI 전환**: LanguageSelector 컴포넌트
- **브라우저 언어 자동 감지**: 최초 방문 시 자동 설정
- **localStorage 저장**: 언어 설정 유지
- **70+ 번역 키**: 검색, 결과, 지도, 에러 메시지

### 5. 오프라인 지원 📴 (v0.50.0+)
- **IndexedDB 기반 캐시**: Dexie.js 활용
- **최근 검색 결과 자동 저장**: 24시간 TTL
- **오프라인 조회 가능**: 네트워크 없이 캐시된 결과 확인
- **Cache-First 전략**: 네트워크 우선 → 캐시 폴백
- **캐시 관리 UI**: 설정 페이지에서 캐시 크기 확인/삭제

### 6. 개인화 추천 🎯 (v0.46.0+)
- **검색 히스토리 기반**: 사용자별 맞춤 카테고리 추천
- **빈도 점수**: 카테고리별 사용 횟수 (0-40 points)
- **최신성 점수**: 마지막 사용 시간 기반 (0-30 points)
- **시간대 점수**: 현재 시간대별 추천 (0-30 points)
- **"자주 찾는 카테고리" 섹션**: 상위 3개 표시
- **"지금 이런 곳 어때요?" 섹션**: 시간대별 추천

### 7. 접근성 ♿ (v0.45.0+)
- **WCAG 2.1 AA 준수**: 국제 접근성 표준
- **ARIA 속성**: role, aria-label, aria-live
- **스크린 리더 지원**: .sr-only 클래스
- **키보드 네비게이션**: :focus-visible 스타일
- **터치 타겟 크기**: 48x48px 최소 (iOS 권장)

### 8. PWA 지원 📱 (v0.42.0+)
- **홈 화면 설치**: InstallBanner 컴포넌트
- **Service Worker**: 정적 자산 캐싱 (sw.js)
- **오프라인 페이지**: 네트워크 실패 시 안내
- **아이콘 세트**: 192x192, 512x512, SVG
- **iOS 최적화**: apple-touch-icon, status-bar-style
