    ...
    <div data-testid="mobile-home-shell" className="md:hidden">
      ...
      {(hasResults || error) && !isLoading && (
        <section
          ...
          className="absolute inset-x-0 bottom-0 z-[1000] isolate max-h-[62dvh] overflow-hidden rounded-t-[2rem]"
          style={{
            paddingBottom: 'env(safe-area-inset-bottom)',
            background: 'var(--bg-surface)',
            border: '1px solid var(--border-soft)',
            borderBottom: 'none',
            boxShadow: '0 -24px 70px -34px rgba(0,0,0,0.65)',
          }}
        >
          <div className="flex justify-center pt-2" aria-hidden="true">
            <div className="h-1.5 w-12 rounded-full" style={{ background: 'var(--border-soft)' }} />
          </div>
          <div className="px-4 pb-3 pt-2" style={{ borderBottom: '1px solid var(--border-soft)' }}>
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="text-xs font-extrabold uppercase tracking-[0.08em]" style={{ color: 'var(--accent)' }}>추천 경유지</p>
                <h2 className="mt-1 text-xl font-extrabold leading-tight" style={{ color: 'var(--text-strong)' }}>
            ...
            {hasResults && (
              <div className="mt-3 grid grid-cols-5 gap-1.5 text-center text-[10px] font-bold" style={{ color: 'var(--text-muted)' }}>
                {['출발', '초반', '중간', '후반', '도착'].map((label, index) => (
                  <span key={label} className="rounded-full py-1" style={{ background: index === 2 ? 'color-mix(in srgb, var(--accent) 15%, var(--bg-surface-muted) 85%)' : 'var(--bg-surface-muted)', color: index === 2 ? 'var(--accent)' : 'var(--text-muted)' }}>
                    {label}
                  </span>
                ))}
              </div>
          ...
          <div data-testid="mobile-result-list" className="max-h-[calc(62dvh_-_114px_-_env(safe-area-inset-bottom))] overflow-y-auto px-4 py-3 scrollbar-hide">
            {error ? (
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        boxShadow: index === 0 ? '0 16px 40px -24px rgba(var(--color-accent-rgb), 0.9)' : 'var(--shadow-1)',
                      }}
                    >
                      <div className="flex items-start gap-3">
                        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm font-extrabold" style={{ background: index === 0 ? 'var(--accent)' : 'var(--bg-surface-muted)', color: index === 0 ? 'var(--text-on-accent)' : 'var(--text-secondary)' }}>
                          {index + 1}
                        </span>
                        <div className="min-w-0 flex-1">
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              <h3 className="truncate text-base font-extrabold" style={{ color: 'var(--text-strong)' }}>{result.place.name}</h3>
                              <p className="mt-1 line-clamp-2 text-xs font-medium leading-snug" style={{ color: 'var(--text-muted)' }}>{address}</p>
                            </div>
                            {index === 0 && (
                              <span className="rounded-full px-2.5 py-1 text-[10px] font-extrabold" style={{ background: 'var(--accent)', color: 'var(--text-on-accent)' }}>
                                BEST
                              </span>
                            )}
                          </div>
                          <div className="mt-3 grid grid-cols-3 overflow-hidden rounded-2xl" style={{ border: '1px solid var(--border-soft)' }}>
                            {[
                              ['추가', formatMin(result.detourCost.duration)],
                              ['거리', formatKm(result.detourCost.distance)],
                              ['점수', `${Math.round(result.finalScore)}`],
                            ].map(([label, value]) => (
                              <span key={label} className="px-2 py-2 text-center" style={{ background: 'var(--bg-surface-muted)', borderRight: label !== '점수' ? '1px solid var(--border-soft)' : 'none' }}>
                                <span className="block text-[10px] font-bold" style={{ color: 'var(--text-muted)' }}>{label}</span>
                                <span className="mt-0.5 block text-sm font-extrabold tabular-nums" style={{ color: 'var(--text-strong)' }}>{value}</span>
                              </span>
                            ))}
                          </div>
                          <div className="mt-3 flex min-h-10 items-center justify-center gap-2 rounded-2xl text-sm font-extrabold" style={{ background: selected ? 'var(--accent)' : 'color-mix(in srgb, var(--accent) 10%, var(--bg-surface) 90%)', color: selected ? 'var(--text-on-accent)' : 'var(--accent)' }}>
