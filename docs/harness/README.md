...
| [goal-loop.md](goal-loop.md) | Ralph Loop 계열의 목표 지속 실행을 MidWayDer식 Goal Contract, 증거 확인, 정지 조건으로 운영하는 방식 | 최우선 |
...
[goal-loop.md](goal-loop.md)는
Ralph Loop 계열의 장기 목표 실행을 MidWayDer에서 어떻게 Goal Contract, slice, evidence check, Human Review handoff로 제한해서 쓸지 정리한다.
